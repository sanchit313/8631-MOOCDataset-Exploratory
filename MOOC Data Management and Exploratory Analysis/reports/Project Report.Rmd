...
Using copy-paste, we will load our 7 enrolment files and1 cyber.security.3_video-stats  by saving them in the 'Data' Folder of our Project Template.

We will check the columns of one of the enrolment files as below:
```{r}
...
```
We notice that they are in a country code format, we will be later modifying them as a part of data preparation.
Most of the remaining columns like gender, age range, employment_status have most values as unknown, hence we will drop them from our analysis.
...
Our interest is in the relation of the types of video and their lengths compared to their number of views, downloads, transcript views, caption views. The other set we will be using is of the columns that give the percentage of viewership across different sets of devices(television, laptop etc). We will drop the remaining columns deemed unnecessary for our analysis.


#### EXPLORE DATA
For doing some basic exploration for our data, we will calculate the number of rows in all our enrollment files.
```{r}
...
We find out that the enrollments are not consistent for all the 7 cohorts.

Calculating the Rows for the video stats file
```{r}
nrow(cyber.security.3_video.stats)
```
There are 13 rows for each column. This tells us that there are 13 different videos at different steps throughout the complete course for which we have the data.

###DATA QUALITY
For the Data files we have selected , we want to check the quality of the data and verify that it is appropriate for the type of analysis we will perform.
...
We will check for any null values in the video sets, and in the detected_country column for any one of the enrolment set.
```{r echo=FALSE}
...
```
We will ignore the NULL in the countries as compared to over 3000 enrollments in each set, they will have negligible impact.
...
##### In countrycode(country_new, "ecb", "country.name") :
