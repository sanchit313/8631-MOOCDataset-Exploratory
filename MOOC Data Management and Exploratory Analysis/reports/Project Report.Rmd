...
Our model displays the number of enrollments per country in an inverted bar chart. Our data mining aim was to determine demographics that reflect the enrollment count. As from the chart, we can see that the United Kingdom has a vast majority of learners (higher from the second country by alot), that is common across all cohorts.
In some cases, India leads where as in others, United States leads. However, our concern to determine the majority enrollment countries is satisfied. This can be used to plan places where there is more potential to increase the enrollments or publicize more.
The existing countries with great number of enrollments are likely to have a further increase and would require more bandwidths.
